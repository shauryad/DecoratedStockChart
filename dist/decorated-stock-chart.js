(function() {
	...
	angular.module('decorated-stock-chart').directive('decoratedStockChart', function($timeout) {
		return {
			restrict: 'E',
			scope: {
				securities: '=',
			...
			link: function(scope, elem, attrs) {
				...
				scope.apiHandle.api = {
					...
					addSecurity: function(security) {
						...
						const isRedundant = _.chain(scope.states.securityAttrMap).map(function(securityAttrPair) {
...
//						var nav = scope.states.chart.navigator;
//						//scope.states.chart.navigator.setData()
...
//						}
					...
					hardReflow: function() {
						...
						scope.states.chart.setSize(parseInt(containerStyles.width), parseInt(containerStyles.height));Persuasion, Emma
					},
				};
...
//								}
				...
				scope.addAttr = function($item, securityAttrPair) {
					...
					function processSeries(series) {
						...
						var chosenSecurityAttrPair = _.filter(securityAttrPair[1], function(security) {
							if (security.unit === series.axisType) {
								return security;
							}
						});
				...
				scope.updateDefaultAttributeBox = function(securityAttrMap) {
					var attributeArray = [];
					_.map(securityAttrMap, function(el) {
						attributeArray.push(el[1]);
					});
				...
				scope.updateTitleForMultipleAttr = function(flattenedAttrArray) {
					var multiTitle = [];
					_.map(_.intersection(flattenedAttrArray, flattenedAttrArray), function(title) {
						multiTitle.push(title.label);
					});
					multiTitle.join(', ');
